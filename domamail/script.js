...
document.addEventListener('DOMContentLoaded', () => {
    console.log('%c Doma Mail %c v1.2.0 %c', 'background:#6366f1;color:#fff;padding:5px;border-radius:5px 0 0 5px;font-weight:bold', 'background:#22d3ee;color:#000;padding:5px;border-radius:0 5px 5px 0', '');
    ...
    clearHistoryBtn.addEventListener('click', () => {
        if (confirm('هل أنت متأكد من مسح السجل؟')) {
            emailHistory = [];
            localStorage.removeItem('domamail_history');
            renderHistory();
        }
    });
